...
export function VibeScore({ transactions, income, expenses }) {
  ...
  useEffect(() => {
    ...
    const spendingToIncomeRatio = expenses / (income || 1);
    ...
    // Calculate uncategorized transactions ratio
    const uncategorizedTransactions = transactions.filter(
      tx => tx.category === 'Uncategorized' && tx.amount > 0
    ).length;
    ...
    const coefficientOfVariation = stdDeviation / meanDailySpending;
    ...
    // Determine vibe message
    let vibeMessage, vibeEmoji;
  ...
  return (
    <Card className="w-full overflow-hidden">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl">Your Vibe Score</CardTitle>
      </CardHeader>
      <CardContent>
